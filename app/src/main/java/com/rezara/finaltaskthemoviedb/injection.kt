package com.rezara.finaltaskthemoviedb

import android.arch.lifecycle.ViewModelProvider
import android.content.Context
import fariborz.rezara.ViewModelFactory.*
import fariborz.rezara.repositories.*
import fariborz.rezara.database.AppDatabase
import fariborz.rezara.database.LocalCache.*
...
object Injection {
    ...
    private fun provideSearchRepository(context: Context): SearchRepository {
        return SearchRepository(NetworkService.instance,
            provideSearchCache(context)
        )
    }
    fun provideSearchViewModelFactory(context: Context): ViewModelProvider.Factory {
        return ViewModelSearchFactory(
    ...
    private fun provideNowShowingRepository(context: Context): NowShowingRepository {
        return NowShowingRepository(NetworkService.instance,
            provideNowShowingCache(
                context
            )
        )
    }
    fun provideNowShowingViewModelFactory(context: Context): ViewModelProvider.Factory {
        return ViewModelNowShowingFactory(
            provideNowShowingRepository(
                context
            )
        )
    }
    ...
    private fun provideUpcomingRepository(context: Context): UpcomingRepository {
        return UpcomingRepository(NetworkService.instance,
            provideUpcomingCache(context)
        )
    }
    fun provideUpcomingViewModelFactory(context: Context): ViewModelProvider.Factory {
        return ViewModelUpcomingFactory(
            provideUpcomingRepository(
                context
            )
        )
    }
    ...
    private fun providePopularRepository(context: Context): PopularRepository {
        return PopularRepository(NetworkService.instance,
            providePopularCache(context)
        )
    }
    fun providePopularViewModelFactory(context: Context): ViewModelProvider.Factory {
        return ViewModelPopularFactory(
            providePopularRepository(
                context
            )
        )
    }
    ...
    private fun provideTopRatedRepository(context: Context): TopRatedRepository {
        return TopRatedRepository(NetworkService.instance,
            provideTopRatedCache(context)
        )
    }
    fun provideTopRatedViewModelFactory(context: Context): ViewModelProvider.Factory {
        return ViewModelTopRatedFactory(
            provideTopRatedRepository(
                context
            )
        )
    }
